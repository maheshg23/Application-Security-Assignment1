...
struct this_gift_card *gift_card_reader(FILE *input_fd)
{
    ...
    while (!feof(input_fd))
    {
        ...
        ptr += 32;
        /* JAC: Something seems off here... */ // MAH char* is stored in int need to change
        ...
        for (int i = 0; i <= gcd_ptr->number_of_gift_card_records; i++)
        {
            ...
            if (gcrd_ptr->type_of_record == 1)
            {
                ...
                /* JAC: something seems off here */ // MAH the amount should be compared // if (gcac_ptr->amount_added < 0)
                if (gcac_ptr < 0)
                    break;
            ...
            if (gcrd_ptr->type_of_record == 2)
            {
                ...
                // advance by the string size + 1 for null
                // BDG: does not seem right // MAH Should be dereferenced to get the actual size of actual record (message) // MAH should we store the message in gcp_ptr->message and then increment ptr? // MAH actual_record is a void pointer so we need to change char * to a void pointer
